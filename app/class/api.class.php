...
class api {
    ...
    public function requestApiByWSLY($newMsg, $msgExtArr = array()) {
        ...
        */
        ...
        $postData = json_encode(array("data" => json_encode($postArr, JSON_UNESCAPED_UNICODE)));
        $reqRet = $this->requestUrl(APP_ORIGIN, $postData, array("Content-Type: application/json"));
    ...
    public function requestApiByNOKNOK($newMsg, $msgExtArr = array()) {
        ...
        if ($extMsgType) {
            if ($extMsgType == "markdown_msg") {
                ...
                $newExtData = array("content" => "@" . $nokNokBot['name'] . " " . $oldMsg, "uid_replied" => $msgSenderUid, "msg_seq" => explode("_", $msgId) [2], "msg_id" => "",);
        ...
        $postArr = array("gid" => $msgGuildId, "target_id" => $msgChannelId, "ts" => $msgTs, "nonce" => $msgNonce);
        ...
        $reqRet = $this->requestUrl($reqUrl, $postData, array("Content-Type: application/json", "Authorization: " . $botInfo['accessToken']));
        if (APP_INFO['debug']) appDebug("output", $postData . "\n\n" . $reqRet);
        return $reqRet;
    }
    /**
    ...
    public function requestApiByQQChannel_1($newMsg, $msgExtArr = array()) {
        ...
        if (strpos($extMsgType, "reply_msg") > - 1) {
            $postMsg[] = array("type" => "reply", "data" => array("id" => $msgId));
        }
        if (strpos($extMsgType, "at_msg") > - 1) {
            $msgAtQQChannel = $newData['msgAtQQChannel'] ?? array();
            $msgAtType = $msgAtQQChannel['at_type'];
            $postMsg[] = array("type" => "at", "data" => array("qq" => $msgAtType == 2 ? "all" : $msgSender));
        ...
        if (strpos($extMsgType, "image_msg") > - 1) {
            ...
            $postMsg[] = array("type" => "image", "data" => array("file" => $extMsgImgUrl));
        }
        //可以叠加
        if ($extMsgType == "json_msg") {
            $postMsg[] = array("type" => "json", "data" => array("data" => $newMsg));
        }
        if ($extMsgType == "xml_msg") {
            $postMsg[] = array("type" => "xml", "data" => array("data" => $newMsg));
        }
        if ($newMsg) {
            $postMsg[] = array("type" => "text", "data" => array("text" => $newMsg));
        }
        $postArr = array("guild_id" => $msgGuildId, "channel_id" => $msgChannelId, "message" => $postMsg);
        $postData = json_encode($postArr);
    ...
    public function requestApiByQQChannel_2($newMsg, $msgExtArr = array()) {
        ...
        $reqRet = $this->requestUrl($reqUrl, $postData, array("Content-Type: application/json", "Authorization: Bot " . $botInfo['id'] . "." . $botInfo['accessToken']));
        if (APP_INFO['debug']) appDebug("output", $postData . "\n\n" . $reqRet);
        return $reqRet;
    }
    /**
    ...
    public function appHandle($ret, $msg = "") {
        $ret = json_encode(array("Ret" => (string)$ret, "Msg" => !$msg ? "" : $msg), JSON_UNESCAPED_UNICODE);
        echo $ret;
    }
    /**
     *
    ...
    public function appGetShortUrl($longUrl) {
        $reqRet = $this->requestUrl(APP_API_APP . "?type=getShortUrl", "url=" . urlencode($longUrl));
        $resJson = json_decode($reqRet);
    ...
    public function appDownloadImg($msgSender, $keywords, $inputPath, $outputPath, $imgUrl = NULL, $imgData = NULL) {
        ...
        return array("name" => $newImgName, "url" => $ret);
    }
    /** 
     *
     * desription 压缩图片
    ...
    public function imgNewSize($imgSrc, $imgDist) {
        ...
        $newHeight = $height;
        switch ($type) {
            case 1 : $giftype = $this->imgCheckGif($imgSrc);
            if ($giftype) {
...
public function utf8Unicode($c) {
    switch (strlen($c)) {
        case 1 : return ord($c);
    case 2 : $n = (ord($c[0]) & 0x3f) << 6;
    $n+= ord($c[1]) & 0x3f;
    return $n;
case 3 : $n = (ord($c[0]) & 0x1f) << 12;
$n+= (ord($c[1]) & 0x3f) << 6;
$n+= ord($c[2]) & 0x3f;
return $n;
case 4 : $n = (ord($c[0]) & 0x0f) << 18;
$n+= (ord($c[1]) & 0x3f) << 12;
$n+= (ord($c[2]) & 0x3f) << 6;
$n+= ord($c[3]) & 0x3f;
return $n;
