...
class genshinSlip_actions extends app
{
    ...
    function EventFun($msg)
    {
        ...
        if (!$this->redisExists("plugins-slip-" . $msgReceiver = $msg['Receiver'])) {
            $expireTime = mktime(04, 00, 00, date("m"), date("d"), date("Y")) + 86400;
            $this->redisSet("plugins-slip-" . $msgReceiver = $msg['Receiver'], $slip[array_rand($slip)], $expireTime);
            $ret = null;
        };
        $ret .= "\n" . base64_decode($this->redisGet("plugins-slip-" . $msgReceiver = $msg['Receiver']));
